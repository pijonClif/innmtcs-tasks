...
def add_to_cart(product_id: int, quantity: int):
    product = None
    for p in products: #find product in products list
        if p["id"] == product_id:
            product = p
            break
    
    if product is None: #product not found
        raise HTTPException(status_code=404, detail="Product not found")
    
    if product["in_stock"] == False: #out of stock
        raise HTTPException(status_code=400, detail=f'{product["name"]} is out of stock')
    
    for item in cart: #check if product already in cart
        if item["product_id"] == product_id:
            item["quantity"] += quantity #update quantity
            item["subtotal"] = item["quantity"] * item["unit_price"] #recalculate subtotal
    ...
    subtotal = product["price"] * quantity #if item not in cart → add new item
...
def view_cart():
    ...
    for item in cart: #loop through cart items and add subtotal
...
def checkout(customer_name: str, delivery_address: str):
    ...
    for item in cart: #Create one order per cart item
        grand_total += item["subtotal"]
        order = {
